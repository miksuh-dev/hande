...
export default class DispatchStream extends WritableStream {
  ...
  open() {
    ...
    this.processInterval = setInterval(this._processAudioBuffer.bind(this), Constants.Audio.frameLength)
  }
  ...
  _processAudioBuffer() {
    if (!this.lastWrite || this.lastWrite + 20 * Constants.Audio.frameLength < Date.now()) {
