...
class Util {
  ...
  static adjustNetworkBandwidth(bitspersec: number) {
    let frames = Network.framesPerPacket;
    let bitrate = Network.quality;

    if (this.getNetworkBandwidth(bitrate, frames) > bitspersec) {
      while (
        bitrate > 8000 &&
        this.getNetworkBandwidth(bitrate, frames) > bitspersec
      ) {
        bitrate -= 1000;
      }
    }
